...
function setTableBodyRows(list){
	...
	var tds='<td><input type="radio" name="checkedItem" value="[id]">1</td>'+
	'<td>1</td>'+
	'<td>1</td>'+
	'<td>1</td>'+
	'<td>1</td>'+
	...
	'<td><button type="button" class="btn btn-default btn-xs btn-valid" value="[validchange]">[stateStr]</button></td>';
	for(var i in list){
		var state = list[i].state==0?'<span class="label label-danger">禁用</span>':'<span class="label label-success">启用</span>';
		var stateStr = list[i].state==0?'启用':'禁用';
		var validchange = list[i].state==0?'1':'0';
		var tr=$('<tr></tr>');
